...
const unsigned char A7_ERRORToken[]={"ERROR"};
...
int A7DataConnect() {
	...
	restart:
		...
		if(!(MapForward(A7_buf,A7_buf_SIZE,(unsigned char*)A7_ERRORToken,5) == NULL))
			goto exit;
...
//	char tcp_header_str[] = "GET /gps_control_panel/gps_mapview/addvehiclelocation.php?";	
	...
	Resetbufer(A7_updated_time_str,12);
	Resetbufer(A7_updated_date_str,12);
	...
	printf("\n Lat %s Lang %s Time %s Date %s ", A7_longitude_str,A7_latitude_str,A7_updated_time_str,A7_updated_date_str);
	//sleep(10);
